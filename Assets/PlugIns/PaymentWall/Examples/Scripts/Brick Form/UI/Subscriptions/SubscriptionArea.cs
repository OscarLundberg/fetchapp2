...
public class SubscriptionArea: MonoBehaviour {
	...
	private void ReloadArea() {
		...
		for (int i = 0; i < listSub.Count; i++) {
			...
			transProduct.FindChild("Name").GetComponent<Text>().text = listSub[i].name;
			transProduct.FindChild("TextPrice/ChargeDescription").GetComponent<Text>().text = listSub[i].chargeDescription;
			transProduct.FindChild("TextPrice").GetComponent<Text>().text = listSub[i].price + " USD";

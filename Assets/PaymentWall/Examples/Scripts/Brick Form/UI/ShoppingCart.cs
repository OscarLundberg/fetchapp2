	...
	void OnEnable() {
		...
		if(cartCount == 0) {
			...
			foreach (KeyValuePair<int,VirtualCurrencyModel> pair in vcDict) {
				...
				Transform tParent = transform.FindChild("Cart List/Cart Panel");
				transCart.SetParent(tParent);
				transCart.localScale = Vector3.one;
				transCart.FindChild("Quantity/InputField").GetComponent<InputField>().text = pair.Value.quantity.ToString();
				transCart.FindChild("TextPrice").GetComponent<Text>().text = pair.Value.price.ToString("C2") + " USD";		
				transCart.FindChild("TextName").GetComponent<Text>().text = pair.Value.name;
				transCart.FindChild("TextTotalPrice").GetComponent<Text>().text = (pair.Value.quantity * pair.Value.price).ToString("C2") + " USD";//(getFinalPrice(pair.Value) * pair.Value.Quantity).ToString("C2") + " USD";
				int id = pair.Key;
				
				Button remB = transCart.FindChild("Button Remove").GetComponent<Button>();
				Button minB = transCart.FindChild("Quantity/Button Minus").GetComponent<Button>();
				Button addB = transCart.FindChild("Quantity/Button Plus").GetComponent<Button>();
...
//
	...
	void MinusItem(int id,Transform tCart) {
		...
		tCart.FindChild("TextTotalPrice").GetComponent<Text>().text = CartManager.Instance.GetTotalPriceOfAProduct(id).ToString("C2") + " USD";//(getFinalPrice(np) * np.Quantity).ToString("C2") + " USD";
		tCart.FindChild("Quantity/InputField").GetComponent<InputField>().text = CartManager.Instance.GetQuantityOfAProduct(id).ToString();
	}

	void PlusItem(int id,Transform tCart) {
		CartManager.Instance.PlusProduct(id);
		tCart.FindChild("TextTotalPrice").GetComponent<Text>().text = CartManager.Instance.GetTotalPriceOfAProduct(id).ToString("C2") + " USD";//(getFinalPrice(p) * p.Quantity).ToString("C2") + " USD";
		tCart.FindChild("Quantity/InputField").GetComponent<InputField>().text = CartManager.Instance.GetQuantityOfAProduct(id).ToString();
	}
}
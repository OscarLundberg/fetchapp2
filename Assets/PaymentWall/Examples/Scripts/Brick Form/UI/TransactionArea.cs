...
//			}
		...
		foreach (TransInfo item in transactionList) {
			...
			transCart.FindChild("TextName").GetComponent<Text>().text = item.ItemName;
			transCart.FindChild("TextPrice").GetComponent<Text>().text = item.Price.ToString("C2") + " USD";
			transCart.FindChild("TextQuantity").GetComponent<Text>().text = item.Quantity.ToString();
			transCart.FindChild("TextTotalPrice").GetComponent<Text>().text = item.TotalPrice.ToString("C2") + " USD";
			if(item.DiscountPrice != 0) {
				transCart.FindChild("TextTotalPrice/TextSaleValue").gameObject.SetActive(true);
				transCart.FindChild("TextTotalPrice/TextSaleValue").GetComponent<Text>().text = item.DiscountPrice.ToString("C2") + " USD";
				transCart.FindChild("TextName/TextSalePercent").gameObject.SetActive(true);
				transCart.FindChild("TextName/TextSalePercent").GetComponent<Text>().text = "SALE! " + item.DiscountPercent + "% Off";
			}
		}
	}
}

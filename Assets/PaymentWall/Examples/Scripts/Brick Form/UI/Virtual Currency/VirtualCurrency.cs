...
public class VirtualCurrency : MonoBehaviour {
	...
	private void ReloadArea() {
		...
		for (int i = 0; i < listVirtual.Count; i++) {
			...
			transProduct.FindChild("Layout/Value").GetComponent<Text>().text = listVirtual[i].name;
			if(listVirtual[i].bonusValue != 0) {
				transProduct.FindChild("Bonus Shard").GetComponent<Text>().text = string.Format(bonusStr,listVirtual[i].bonusValue);
			}
			transProduct.FindChild("TextPrice").GetComponent<Text>().text = string.Format (priceStr,listVirtual[i].price);

			Button b = transProduct.FindChild("Button Add To Cart").GetComponent<Button>();
